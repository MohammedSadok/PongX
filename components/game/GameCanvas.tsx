...
import { calculateWordWidth, normalizeBallVelocity, playCollisionSound } from "./utils"
...
export const GameCanvas = memo(
  function GameCanvas({ activeSection, setActiveSection }: GameCanvasProps) {
    ...
    const initializeGame = useCallback(() => {
      ...
      const totalWidthLarge = calculateWordWidth(words[0], LARGE_PIXEL_SIZE, PIXEL_MAP, LETTER_SPACING)
      ...
      words.forEach((word, wordIndex) => {
        ...
        const totalWidth =
          wordIndex === 0
            ? calculateWordWidth(word, adjustedLargePixelSize, PIXEL_MAP, LETTER_SPACING)
            : words[1].split(" ").reduce((width, w, index) => {
                return (
                  width +
                  calculateWordWidth(w, adjustedSmallPixelSize, PIXEL_MAP, LETTER_SPACING) +
    ...
    const updateGame = useCallback(() => {
      ...
      if (ball.isActive) {
        ...
        if (ball.isUserControlled && ball.y - ball.radius < navbarHeightRef.current) {
        ...
        if (ball.y - ball.radius < navbarHeightRef.current && !ball.isUserControlled) {
          ball.y = navbarHeightRef.current + ball.radius
          ball.dy = -ball.dy
      ...
      paddles.forEach((paddle) => {
        if (paddle.isVertical) {
          ...
          paddle.targetY = Math.max(0, Math.min((canvasRef.current?.height || 0) - paddle.height, paddle.targetY))
          paddle.y += (paddle.targetY - paddle.y) * 0.1
        } else {
          paddle.targetY = ball.x - paddle.width / 2
          paddle.targetY = Math.max(0, Math.min((canvasRef.current?.width || 0) - paddle.width, paddle.targetY))
    ...
    const drawGame = useCallback(() => {
      ...
      ballRef.current.trail.forEach((point, index) => {
        // Use theme colors
        ctx.fillStyle = `rgba(${
          ballRef.current.isUserControlled ? hexToRgb(currentTheme.activeBallColor) : hexToRgb(currentTheme.ballColor)
        }, ${point.alpha * 0.5})`
        ctx.beginPath()
        ctx.arc(point.x, point.y, ballRef.current.radius * (0.5 + point.alpha * 0.5), 0, Math.PI * 2)
        ctx.fill()
      })
      ...
      const ball = ballRef.current
      ctx.fillStyle = ball.isUserControlled ? currentTheme.activeBallColor : currentTheme.ballColor
      ctx.beginPath()
      ...
      ctx.shadowBlur = 15
      ctx.shadowColor = ball.isUserControlled ? currentTheme.activeBallColor : currentTheme.ballColor
      ctx.beginPath()
      ctx.arc(ball.x, ball.y, ball.radius, 0, Math.PI * 2)
      ctx.fill()
      ctx.shadowBlur = 0
      ...
      navbarRef.current.forEach((item) => {
        ...
        ctx.fillStyle = item.isActive ? currentTheme.navbarActive : currentTheme.navbarBg
        ctx.fillRect(item.x + item.shakeOffset * item.shakeDirection, item.y, item.width, item.height)
        ...
        if (item.isActive) {
          ...
          ctx.fillRect(item.x + item.shakeOffset * item.shakeDirection, item.y + item.height - 3, item.width, 3)
        }
    ...
    }, [resizeCanvas, gameLoop, handleMouseDown, handleMouseMove, handleMouseUp])
